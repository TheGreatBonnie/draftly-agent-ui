...
const runs = [
  ...
  ["run_01H7Y1", "Release v1.2.0 notes", "release_notes", 10, 9, 1, "0.88"],
  ["run_01H6K9", "Redis caching guide", "documentation", 14, 14, 0, "0.96"],
  ["run_01HSQ8", "Support response evaluation", "support", 16, 13, 3, "0.84"],
  ["run_01H4J2", "API reference update", "api_reference", 12, 11, 1, "0.90"],
];
...
const trendSeries: Record<TrendRange, { dates: string[]; scores: number[] }> = {
  ...
  "Last 30 days": {
    dates: Array.from({ length: 30 }, (_, index) =>
      index < 26 ? `Aug ${7 + index}` : `Sep ${index - 25}`,
    ),
    scores: [
      41, 44, 43, 48, 46, 51, 49, 54, 57, 55, 59, 61, 60, 65, 63, 68, 70, 69,
      73, 72, 76, 78, 77, 82, 85, 87, 90, 94, 96, 100,
    ],
  },
};
...
function EvaluationScoreTrend() {
  ...
  const points = series.scores.map((score, index) => {
    const x =
      plotLeft + (index * (plotRight - plotLeft)) / (series.scores.length - 1);
  ...
  const tooltipX = Math.min(
    Math.max(highlight.x - 44, plotLeft),
    plotRight - 88,
  );
  const labelIndexes = Array.from(
    new Set(
      Array.from({ length: 7 }, (_, index) =>
        Math.round((index * (points.length - 1)) / 6),
      ),
    ),
  );
        ...
        <svg
          ...
          <g transform={`translate(${tooltipX} 0)`}>
            ...
            <text
              x="12"
              y="30"
              className="fill-slate-900 text-[10px] font-semibold dark:fill-slate-100">
