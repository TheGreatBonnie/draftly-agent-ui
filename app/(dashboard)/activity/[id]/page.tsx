...
export default async function Page({
  ...
  const item = activityItems.find((x) => x.id === id) || activityItems[0];
  return (
    <>
      <Link
        href="/activity"
      ...
      <PageHeader
        ...
        actions={<Badge>{item.source}</Badge>}
      />
      <Tabs
        active="Overview"
        items={["Overview", "Related activity", "Artifacts", "Raw event"]}
      />
      <div className="mt-4 grid gap-4 lg:grid-cols-[1fr_320px]">
        <Card className="p-5">
          ...
          <div className="relative mt-5 space-y-4 before:absolute before:bottom-4 before:left-[19px] before:top-4 before:w-px before:bg-slate-200">
            {[
              [GitPullRequest, "GitHub event received", "10:24:01"],
              [Bot, "GitHub Intelligence Agent invoked", "10:24:03"],
              [Play, "PR Documentation workflow started", "10:24:08"],
              [FileText, "Documentation change generated", "10:25:01"],
              [ShieldCheck, "Human review created", "10:25:14"],
            ].map(([Icon, label, time]) => (
              <div key={String(label)} className="relative flex gap-3">
                <IconTile size="md">
                  <Icon className="h-5 w-5" />
                </IconTile>
                <div className="flex flex-1 items-center justify-between rounded-xl border border-slate-100 p-3">
        ...
        <aside className="space-y-4">
          <Card className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm font-semibold">Event metadata</div>
